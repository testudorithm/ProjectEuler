...
namespace ProjectEuler
{
    class Problem_11
    {
        ...
        //    matrix = stringToMatrix(matrixStr);
        ...
        public static int[,] stringToMatrix(string x)
        {

            string[] rows = x.Split('\n');
            string[] tempCol = rows[0].Split(' ');

            int row = rows.Length, col = tempCol.Length;
            ...
            for (int i = 0; i < row; i++)
            {
                ...
                for (int j = 0; j < col; j++)
                {
                    ...
                    matrix[i, j] = Convert.ToInt32(Col[j]);
